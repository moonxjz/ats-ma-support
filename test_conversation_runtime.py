...
from conversation_runtime import (ConversationSession, PendingTurn, TurnFailure, TurnStatus,
...
from root_agent import route_message, execute_route, RoutingStatus
from support_agent import (CustomerResponse, SupportAction, SupportKnowledgeContext, KnowledgeFact,
...
from test_order_creation_controller import complete_customer_state
...
class RuntimeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.support_patch = patch('support_agent.chat', side_effect=fake_support_chat)
    ...
    def run_order(self, fail_after_create=False):
        ...
        with patch('order_agent.extract_order_information', side_effect=extracted) as extract, \
             patch('order_agent.interpret_confirmation_response', return_value=ConfirmationInterpretation(intent='CONFIRMED')) as interpret:
    ...
    def test_post_create_support_failure_retry_never_reexecutes_and_commits_once(self):
        ...
        before = deepcopy(session)
        with patch('conversation_runtime.classify_message', side_effect=AssertionError('No classification')), \
             patch('conversation_runtime.route_message', side_effect=AssertionError('No routing')), \
             patch('conversation_runtime.execute_route', side_effect=AssertionError('No dispatch')), \
             patch('order_creation_controller.execute_order_creation_workflow', side_effect=AssertionError('No controller')), \
             patch('order_creation_order_store.create_order', side_effect=AssertionError('No store')):
    ...
    def test_existing_store_idempotent_replay_retains_same_order(self):
        ...
        with patch('order_agent.extract_order_information', return_value=ExtractedOrderInformation()), \
             patch('order_agent.interpret_confirmation_response', return_value=ConfirmationInterpretation(intent='CONFIRMED')):
...
class LiveRuntimeTests(unittest.TestCase):
    ...
    def run_live_conversation(self, *, dense):
        ...
        answers = {
            ...
            'delivery_address.address_line_1': 'My delivery street address is 1 Example Street.',
            'delivery_address.address_line_2': 'There is no second address line.',
        ...
        with TemporaryDirectory() as tmp:
            ...
            for turn_index in range(1, 101):
                ...
                from classifier import classify_message as real_classifier
                from support_agent import chat as real_support_chat
                from support_agent import _validate_response_grounding as real_validate
                from order_creation_extraction import chat as real_extraction_chat
                from order_agent import extract_order_information as real_extract
                from order_agent import apply_extracted_order_information as real_apply
                from order_agent import apply_order_creation_reentry as real_reentry
                from order_agent import execute_order_creation_workflow as real_execute
                trace = {
                ...
                try:
                    with patch('conversation_runtime.route_message', side_effect=capture_route), \
                         patch('order_creation_extraction.chat', side_effect=capture_extraction_chat), \
                         patch('order_agent.extract_order_information', side_effect=capture_extract), \
                         patch('order_agent.apply_extracted_order_information', side_effect=capture_apply), \
                         patch('order_agent.apply_order_creation_reentry', side_effect=capture_reentry), \
                         patch('order_agent.execute_order_creation_workflow', side_effect=capture_execute), \
                         patch('support_agent.chat', side_effect=capture_chat), \
                         patch('support_agent._validate_response_grounding', side_effect=capture_validation):
