...
def ref(text, index=1, quote=None):
    ...
    return e.reference(text, index, start, None if quote is None else start + len(quote)).model_dump(mode='json')
...
class SimulatorTests(unittest.TestCase):
    ...
    def test_exact_payload_and_call(self):
        ...
        self.assertEqual(payload, inputs.model_dump(mode='json'))
    ...
    def test_s02_ask_options_and_select(self):
        ...
        offer = e.availability(support, 3, 'timber')[0].evidence.model_dump(mode='json')
    ...
    def confirmation(self, inputs, final=False):
        ...
        return {'kind': 'CONFIRM_FINAL_ORDER' if final else 'CONFIRM_CONFIGURATION', 'artifact': artifact.evidence.model_dump(mode='json'), 'approval_request': request.model_dump(mode='json')}
    ...
    def test_missing_approval_not_authorized(self):
        ...
        action = {'kind': 'CONFIRM_CONFIGURATION', 'artifact': artifact.evidence.model_dump(mode='json'), 'approval_request': ref(text, quote='Please review the details.')}
        self.reject(self.inputs(text), turn(action))
    ...
    def test_mismatch_public_stop(self):
        ...
        artifact = e.extract_artifact(text, 1)
        result = s.step(self.inputs(text), chat_fn=model({'kind': 'STOP', 'category': 'CONTENT_MISMATCH', 'evidence': [artifact.evidence.model_dump(mode='json')]}))
