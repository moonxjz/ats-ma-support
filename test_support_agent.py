...
from business_result import BusinessResult, BusinessResultReason as Reason, BusinessResultStatus as Status
...
class RequiredInputRenderingTests(unittest.TestCase):
    def setUp(self):
        self.patcher = patch("support_agent.chat")
    ...
    def test_existing_numeric_guard_unchanged_on_free_prose(self):
        from support_agent import _validate_response_grounding
        context = prepare_response_context(result_for())
...
class SupportGenerationTests(unittest.TestCase):
    ...
    def test_no_controller_store_extraction_or_confirmation_calls(self):
        targets = ["order_creation_controller.execute_order_creation_workflow",
                   "order_creation_controller.apply_order_creation_reentry", "order_creation_order_store.create_order",
                   "order_creation_extraction.extract_order_information", "order_creation_confirmation.interpret_confirmation_response"]
...
class LiveSupportTests(unittest.TestCase):
    def setUp(self):
        ...
        from support_agent import chat as live_chat
        def capture(*args, **kwargs):
            response = live_chat(*args, **kwargs)
            print("\nLIVE SUPPORT OUTPUT:", response.message.content, flush=True)
            return response
        patcher = patch("support_agent.chat", side_effect=capture)
        patcher.start()
