...
from order_creation_order_store import (
...
class OrderStoreTests(unittest.TestCase):
    ...
    def test_empty_store_commits_first_order_with_exact_snapshot(self):
        with patch("order_creation_order_store.current_utc_time", return_value="2026-09-08T00:00:00+00:00"):
            result = self.create()
    ...
    def test_idempotent_replay_returns_original_record_unchanged(self):
        with patch("order_creation_order_store.current_utc_time", return_value="2026-09-08T00:00:00+00:00"):
            first = self.create()
        with patch("order_creation_order_store.current_utc_time", return_value="2026-09-09T00:00:00+00:00"):
            replay = self.create()
    ...
    def test_atomic_replace_failure_does_not_report_success_or_replace_store(self):
        ...
        with patch("order_creation_order_store.os.replace", side_effect=OSError("replace failed")):
