...
from tests.test_public_observation import (
...
class SimulatorTests(unittest.TestCase):
    ...
    def test_development_safeguard_with_repeated_approvals(self):
        ...
        source=(Path(__file__).resolve().parents[1]/'evaluation/customer_simulator.py').read_text()
    ...
    def test_boundary_imports_and_no_external_calls(self):
        allowed={'re','html','json','typing','pydantic','workflow.confirmation_presentation','evaluation.scenario_spec','evaluation.public_observation'}
        for name in ['customer_simulator','public_observation']:
            source=(Path(__file__).resolve().parents[1]/'evaluation'/f'{name}.py').read_text()
