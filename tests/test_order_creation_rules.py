...
from workflow.order.order_creation_rules import DEMO_CUE_LENGTH_INCHES, build_configuration_snapshot, validate_room_size, calculate_total_price
...
class ProductAuthorizationTests(unittest.TestCase):
    def test_seven_fields_and_historical_quantity_without_mutation(self):
        from workflow.order.order_creation_rules import confirmed_product_configuration_matches
        from tests.test_order_creation_state import complete_customer_state
        state = complete_customer_state()
        state.quantity = 2
...
class FinalSnapshotRulesTests(unittest.TestCase):
    def state(self):
        from tests.test_order_creation_controller import PricingTests
        from workflow.order.order_creation_controller import handle_pricing
    ...
    def test_prerequisites_reject_without_mutation(self):
        from workflow.order.order_creation_rules import build_final_order_snapshot
