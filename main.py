...
def main():
    ...
    print('Enter /quit to exit. History and workflow state are retained only for this process.')
    while True:
        try:
            message = input('Retry (/retry or /quit) > ' if pending else 'Customer > ')
        ...
        pending = None
        print('Support > ' + result.customer_response.text)
        if args.debug:
