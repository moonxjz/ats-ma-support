...
from support_agent import compose_customer_response, ConfirmationFraming
from test_support_agent import CONFIG, FINAL, Reason, result_for
...
- **Quantity:** 1''')
    ...
    def test_actual_controller_snapshot_serialization_is_rendered_directly(self):
        from test_order_creation_controller import complete_customer_state, PricingTests
...
class ConfirmationCompositionTests(unittest.TestCase):
    def test_artifact_inserted_unchanged_and_llm_sees_no_values_or_history(self):
        for reason, key, renderer in (
            ...
            with patch('support_agent.chat', return_value=SimpleNamespace(message=SimpleNamespace(content=json.dumps(framing)))) as chat:
    ...
    def test_invalid_framing_and_failures_propagate(self):
        ...
        for content in invalid:
            with patch('support_agent.chat', return_value=SimpleNamespace(message=SimpleNamespace(content=content))):
                with self.subTest(content=content), self.assertRaises(ValueError):
                    compose_customer_response(result_for(Reason.FINAL_CONFIRMATION_REQUIRED))
        with patch('support_agent.chat', side_effect=ConnectionError('Unavailable')):
    ...
    def test_missing_artifact_evidence_fails_before_llm(self):
        ...
        with patch('support_agent.chat') as chat, self.assertRaises(ValueError):
    ...
    def test_natural_explicit_placement_question_does_not_require_keyword(self):
        ...
        with patch('support_agent.chat', return_value=SimpleNamespace(message=SimpleNamespace(content=json.dumps(framing)))):
